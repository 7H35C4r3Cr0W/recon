...
def test_add_credential_ok_gated_on_required_fields(qtbot: QtBot) -> None:
    ...
    assert tokens.active_palette().accent in d._ok.styleSheet()  # Ok is the primary action
...
def test_vault_buttons_ranked_and_selection_gated(qtbot: QtBot, tmp_path: Path) -> None:
    ...
    qtbot.addWidget(d)
    assert tokens.active_palette().accent in d._add.styleSheet()  # Add is primary
...
def test_spray_run_is_primary_and_gated(qtbot: QtBot, tmp_path: Path) -> None:
    ...
    qtbot.addWidget(off)
    assert tokens.active_palette().accent in off._run.styleSheet()
