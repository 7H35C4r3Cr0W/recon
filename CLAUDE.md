...
- **Opt-in master switch.** An `exploit_enabled` setting (default `false`) keeps the *public* build recon-only until the user turns execution on (first-Run one-click enable, or Preferences). It is a distribution posture, not a per-command gate — the per-run confirmation is the gate.
...
- **Tied to discovered services/ports.** Each `ServiceExploits` declares the `ports` that signal its presence; the picker surfaces the surfaces open on *this* box first (mirrors Recon's service focus).
