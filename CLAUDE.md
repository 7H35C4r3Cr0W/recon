...
### 2b. Exploitation tab — owner-authorized, GUI-initiated, opt-in execution, human-confirmed
...
**OSCP-legality (fact-checked, keep current):** OffSec's exam bans *automated-exploitation frameworks* and specific tools — **Metasploit** (one manual use total; out of this tool entirely), **SQLMap**, and **commercial/automated scanners** (Nessus, Burp Pro, Acunetix, Qualys) — plus **any AI/LLM assistance during the exam**. It **allows** manual exploitation with attack scripts/tools: impacket, evil-winrm, netexec, responder, certipy, hashcat/john, public exploit PoCs, etc. So a per-command, user-initiated, single-shot attack is exam-legal; automated multi-step exploitation, sqlmap, Metasploit, commercial scanners, and AI are not. That line is enforced in code (below), not by convention.

Rules (non-negotiable):

- **Opt-in execution, OFF BY DEFAULT.** An `exploit_enabled` setting (default `false`) gates execution, mirroring Spray mode (§2a). With it off the tab is **shown-only** (build + Copy + parse pasted output). The first Run offers a one-click enable; it is also in Preferences. With it off, `shell.run` blocks every exploit tool exactly as today. **The default install stays recon-only + exam-legal.**
- **No blind attacks — every Run confirms.** A Run is only ever user-initiated (the user picked the action + clicked Run), and pops a confirmation naming the target before anything executes. There is **no auto-run, no chaining, no "scan → exploit" pipeline.** One command per Run.
- **Execution is gated at the exec chokepoint.** Run goes through `shell.run(..., exploit=True)` → `policy_violation(exploit=True)`, which permits the `EXPLOIT_TOOLS` allow-list *only* in exploit mode. **`_HARD_FORBIDDEN_TOOLS` (sqlmap, msfconsole/msfvenom/msfdb/meterpreter/metasploit, armitage, cobaltstrike, nessus/openvas/nexpose, burpsuite, acunetix) is checked *ahead of every allow-list* and blocked in ALL modes** — they can never run, even hand-typed. No runtime LLM calls; no PoC download/execute/transform (Exploit-DB stays lookup + linkout, §14).
