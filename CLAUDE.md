...
- **Tied to discovered services/ports.** Each `ServiceExploits` declares the `ports` that signal its presence; the picker surfaces the surfaces open on *this* box first (`●`-marked, mirrors Recon's service focus) and binds `{port}`/`{url}` to the actual discovered port. Selecting an action for a service the scan did **not** find shows a loud, non-blocking warning ("⚠ … was NOT found on this target by the scan") — it never disables Run (the human confirm stays the guardrail per this section), it just tells the operator they're acting on a service discovery didn't see.
